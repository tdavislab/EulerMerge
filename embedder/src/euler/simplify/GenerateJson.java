...
public class GenerateJson {
	...
	public String jsonOutput(int sep) {
		...
		ret += "\t\"zoneGraph\": [";
		// get all edges
		for(Edge e : simplify.getDualGraph().getEdges()) {
			String fLabel = e.getFrom().getLabel();
			String tLabel = e.getTo().getLabel();
			if(fLabel==""){
				fLabel = "0";
			}
			if(tLabel==""){
		...
		concreteDiagram.setConcurrentOffset(sep); // I change the offset to be 0
		concreteDiagram.generateContours();

		// concreteDiagram.setConcurrentOffset(ConcreteDiagram.CONCURRENT_OFFSET); // I change the offset to be 0
		ret += "\n\t\"curves\" :[";
