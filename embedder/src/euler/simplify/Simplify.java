...
public class Simplify {
	...
	public static void main(String[] args) {
		...
		// examples of set systems
		ad = new AbstractDiagram("0 a b c d e f g bd bg ce eg bde beg deg abdf");
		...
		DiagramDrawerPlanar.timeOutMillis = 10000;
...
//			System.out.println(gs.jsonOutput());
		...
		// save the final simplified unsmoothed Euler Diagram
		simplify.saveCurrentEulerDiagram(simplify);	// save the euler diagram
	 ...
	 * @return
	 */
	public Simplify clone(Simplify simplifyObj) {
		AbstractDiagram ad = new AbstractDiagram("0 a b c d e bcd cde abcde"); //Zhmutski, Peter
		Simplify spl = new Simplify(ad);
		
		spl.abstractDiagram = simplifyObj.getAbstractDiagram().clone();
		spl.dualGraph = simplifyObj.getDualGraph().clone();
		return spl;
	}

	/**
	 * save the euler diagram into json file
	 * @param simplifyObj simply object
	 */
	public void saveCurrentEulerDiagram(Simplify simplifyObj){
		// get a nice layout
		PlanarForceLayout pfl = new PlanarForceLayout(simplifyObj.getDualGraph());
		pfl.drawGraph();
		simplifyObj.getDualGraph().fitInRectangle(100,100,400,400); 

		Simplify simplifyClone = simplifyObj.clone(simplifyObj);
		GenerateJson gsG = new GenerateJson(simplifyClone);
		String jsonoutput = gsG.jsonOutput(0);
		simplifyObj.saveJSONtoFile("EulerJSON-0", jsonoutput);
		simplifyClone = simplifyObj.clone(simplifyObj);
		gsG = new GenerateJson(simplifyClone);
		jsonoutput = gsG.jsonOutput(2);
		simplifyObj.saveJSONtoFile("EulerJSON-2", jsonoutput);
	}

	/**
	 * save a string into a file
	 * Note: the euler digram results will be saved into the smoothEuler/data
	 */
	public void saveJSONtoFile(String filename, String jsonStr){
		try {
			FileWriter myWriter = new FileWriter("../smoothEuler/static/data/eulerData/"+filename+".json");
